...
public class SceneManager {
    ...
    private final Array<Scene> sceneBackStack = new Array<>();
    ...
    public void __dispose () {
        ...
        if (sceneBackStack.contains(currScene, true))
            sceneBackStack.removeValue(currScene, true);

        if (sceneBackStack.contains(nextScene, true))
            sceneBackStack.removeValue(nextScene, true);

        for (Scene scene : sceneBackStack)
            scene.__destroy();
    ...
    /**
     * Reverts to the previous stackable scene.
    ...
    protected void pushScene(Scene scene) {
        if(sceneBackStack.contains(scene, true))
            return;

        sceneBackStack.add(scene);
    }
    ...
    protected Scene popScene() {
        if(sceneBackStack.isEmpty())
            return null;

        return sceneBackStack.pop();
    }
